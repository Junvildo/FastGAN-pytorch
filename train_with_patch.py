...
from model_patch import weights_init, NLayerDiscriminator, PixelDiscriminator
...
def train_d(netD, images, label="real"):
    netD.zero_grad()
    pred = netD(images)
    if label == "real":
        err = torch.mean((pred - 1)**2)
    else:
        err = torch.mean(pred**2)
    err.backward()
    return err.item()
...
def train(args):
    ...
    if use_pixel:
        netD = PixelDiscriminator(input_nc=3, ndf=ndf, im_size=im_size)
    else:
        netD = NLayerDiscriminator(input_nc=3, ndf=ndf, im_size=im_size)
    ...
    for iteration in tqdm(range(current_iteration, total_iterations+1)):
        ...
        if iteration % (save_interval*50) == 0:
            ...
            with torch.no_grad():
                ...
                vutils.save_image(F.interpolate(real_image, 128).add(1).mul(0.5), saved_image_folder+'/rec_%d.jpg'%iteration )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_pixel', type=int, default=0, help='to use pixel PatchGAN or NLayersPatchGAN, 1=Pixel PatchGAN, 0=NLayersPatchGAN')
